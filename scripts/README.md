...
3. **Install dependencies:**
   ...
   npm install
   ```
4. **Bootstrap Vercel + set repo secret:**
   ```powershell
   .\scripts\bootstrap-vercel-project.ps1
   ```
   Or link and trigger first deploy in one go:
   ```powershell
   .\scripts\bootstrap-vercel-project.ps1 -TriggerDeploy
   ```
   Full setup (bootstrap → verify runner → deploy):
   ```powershell
